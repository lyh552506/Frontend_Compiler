...
std::vector<std::pair<CallInst *, RetInst *>>
TailRecurseEliminator::CanTailRecurseTransform(Function *func) {
  ...
  for (auto usr : usrlist) {
    ...
    if (call->GetParent()->GetParent() == func) {
      ...
      if (ret != nullptr) {
        if (ret->GetUserListSize() ==
                0 // 函数返回void，就算 call 有返回值 caller saved 无所谓
...
TailRecurseEliminator::SeparateAllocaIntoEntryBlock() {
